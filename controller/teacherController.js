const { teacherServices, attendenceServices } = require("../services");
...
exports.getTodayAttendance = async (req, res, next) => {
    // try {
    //     const request = req.body;
    //     console.log("requestrid", request);

    //     request.userId = req.userId || "mock-user"; // or from req.user
    //     const result = await attendenceServices.getTeacherTodayAttendance(request);
    ...
    try {
        const request = req.body;
        const reportData = await attendenceServices.getTeacherTodayAttendance(request);
        return formatResponse(res, reportData);
    } catch (error) {
        next(error)
    }
...
exports.clockIn = async (req, res, next) => {
    try {
        const request = req.body;
        request.userId = request.userId || "mock-user";
        const result = await attendenceServices.clockInTeacher(request);
        return formatResponse(res, result);
    } catch (error) {
        next(error);
    }
...
exports.clockOut = async (req, res, next) => {
    ...
    try {
        const request = req.body;
        request.userId = request.userId || "mock-user";
        const result = await attendenceServices.clockOutTeacher(request);
        return formatResponse(res, result);
    } catch (error) {
        next(error);
    }
