...
if (scannedRes.length > 0) {
    const firstContent = scannedRes.find(item => item?.content)?.content;

    if (firstContent) {
        let initialDetails = await helper.extractValuesFromInput(firstContent);
        const testId = initialDetails.find(item => item.label === 'Test ID')?.value;
        const rollNo = initialDetails.find(item => item.label === 'Roll No')?.value;
        ...
        if (testId && rollNo) {
            request.data = {
                ...request.data,
                class_test_id: request.data.test_id,
            ...
            console.log("classTestData:--", classTestData)
            if (helper.isEmptyObject(classTestData.Item)) {
                console.log(constant.messages.COULDNT_READ_TEST_ID);
            ...
            if (studentData.Items.length === 0) {
                console.log(constant.messages.COULDNT_READ_ROLL_NUMBER);
                return;
            }

            request.data.student_id = studentData.Items[0].student_id;
            console.log("studentData.Items[0].student_id---", studentData.Items[0].student_id)

            const testResultData = await testResultRepository.fetchTestDataOfStudent2(request);

            for (let i = 0; i < scannedRes.length; i++) {
                const scannedItem = scannedRes[i];
                console.log(`OPENAI scanned Data [${i}]:`, scannedItem);

                if (scannedItem?.content) {
                    let pageDetailsRes = await helper.extractValuesFromInput(scannedItem.content);
                    const answers = await helper.extractAnswersFromInput(scannedItem.content);
                    const pageNo = pageDetailsRes.find(item => item.label === 'pageNo')?.value;

                    if (pageNo) {
                        if (!Array.isArray(pageMetadata)) pageMetadata = [];

                        const existingPageIndex = pageMetadata.findIndex(meta => meta.page_no === pageNo);

                        const newPageData = {
                            page_no: pageNo,
                            url: request.data.Keys[i],
                            confidence_rate: 0,
                            studentAnswer: answers
                        };
                        if (existingPageIndex !== -1) {
                            pageMetadata[existingPageIndex] = newPageData;
                        } else {
                            pageMetadata.push(newPageData);
                        }
                    } else {
                        console.log(`Index [${i}]:`, constant.messages.COULDNT_READ_PAGE_DETAILS);
                    }
                } else {
                    console.log(`Index [${i}]:`, constant.messages.COULDNT_EXTRACT_TEXT);
                }
            }
            ...
            if (testResultData.Items.length === 0) {
                request.data.answer_metadata = pageMetadata;
                ...
                if (insertResponse.$metadata.httpStatusCode === 200) {
                    ...
                    console.log("New Student Record Not Added");
                }
            } else {
                let existingMetadata = testResultData.Items[0].answer_metadata || [];

                pageMetadata.forEach(newPage => {
                    const existingPage = existingMetadata.find(meta => meta.page_no === newPage.page_no);
                    if (existingPage) {
                        Object.assign(existingPage, newPage);
                    } else {
                        existingMetadata.push(newPage);
                    }
                });

                existingMetadata.sort((a, b) => a.page_no - b.page_no);

                const updateRequest = {
                    data: {
                        result_id: testResultData.Items[0].result_id,
                        answer_metadata: existingMetadata
                    }
                };
            ...
            console.log(constant.messages.COULDNT_READ_PAGE_DETAILS);
        }
    } else {
        console.log(constant.messages.COULDNT_EXTRACT_TEXT);
    }
} else {
    console.log("No scanned results found.");
}
